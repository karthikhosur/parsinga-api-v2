...
edu_uni_head = ["university", "college" , "institute"]
    ...
    if sec_data:
        for i in range(len(sec_data)):
                ...
                try:
                    ...
                    if re.search(only_college_degree,sec_data[i][2]):
                        if re.search(only_college_degree,sec_data[i][2])[0] not in exclude_degree_names:
                            education_dict['degree'].append(re.search(only_college_degree,sec_data[i][2])[0])
                except : 
                    a = 0
    if sec_data:
        edu_text = ""
        for i in range(len(sec_data)):
            ...
            if len(sec_data[i][2].split(" ")) > 2 :
                ...
                try:
                    ...
                    if re.search(r'([0-9][.][0-9]*|[0-9]{2}[.][0-9]*|[0-9]{2}%)', sec_data[i][2]):
                        match = re.search(r'([0-9][.][0-9]*|[0-9]{2}[.][0-9]*|[0-9]{2}%)', sec_data[i][2])[0]
                        education_dict["grade"].append(match)
                except:
                    a = 0

        
        dates_years = duration_func(all_terms_text)
        total_years = dates_years
        try:
            for i in range(len(education_dict["degree"])):
                education_dict["dates"]["date_tags"].append(total_years[i])
        except:
